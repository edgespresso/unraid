...
echo ""
# Zip the USB files and LIBVIRT image
echo "Zipping UNRAID USB files and the LIBVIRT image..."
zip -rq $backup_path/unraid_backup-$(date +%d-%b-%Y).zip $usb_path $libvirt_file

echo "Copying today's backup to the remote backup server..."
cp $backup_file $remote_path

echo "Removing backup files that are ovewr 7 days old... "
